...
const TagPreview = (props) => {
  ...
  const taggedPosts = (
    <div className={classes.taggedPosts}>
      {postsElem}
    </div>
  );

  return (
    <Card className={`tag-preview ${classes.root}`}>
      <CardHeader
        title={
          <LinkTo hoverNone href={`/blog/tags?tagSlug=${slug}`} as={`/blog/tags/${slug}`}>{name}</LinkTo>
        }
        subheader={slug}
      />
      <CardMedia
