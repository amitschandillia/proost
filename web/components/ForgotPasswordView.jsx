...
import recoverPassword from '../utils/recover-password';
...
const ForgotPasswordView = (props) => {
  const {
    classes, pageURL, flagEmailError, emailStatus, emailAlreadyExists, emailNotRegistered, showSignIn, flagError,
  } = props;
...
//   };
  ...
  const submitRegisteredEmail = async (e) => {
    const { target: { to: { value: to } } } = e;
    if (!emailValidator.validate(to)) { flagError(); } else {
        // email is valid; check if registered
        // if not registered, call emailNotRegistered
        // if registered, send recovery email
    }
  };

  return (
    <>
      <form onSubmit={(e) => { e.preventDefault(); submitRegisteredEmail(e); }}>
        <EmailField
          name="to"
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles)(ForgotPasswordView));
